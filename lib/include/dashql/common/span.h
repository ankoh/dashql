...
template <typename C> inline span_constexpr auto size(C const& cont) -> decltype(cont.size()) { return cont.size(); }

template <typename T, std::size_t N> inline span_constexpr auto data(T (&arr)[N]) span_noexcept -> T* {
    return &arr[0];
}

template <typename C> inline span_constexpr auto data(C& cont) -> decltype(cont.data()) { return cont.data(); }

template <typename C> inline span_constexpr auto data(C const& cont) -> decltype(cont.data()) { return cont.data(); }
...
template <class T, span_CONFIG_EXTENT_TYPE Extent> struct is_span_oracle<span<T, Extent> > : std::true_type {};
...
template <class C>
struct is_container : std17::bool_constant<!is_span<C>::value && !is_array<C>::value && !is_std_array<C>::value &&
...
span_noreturn inline void report_contract_violation(char const* /*msg*/) span_noexcept { std::terminate(); }
...
template <class T, extent_t Extent /*= dynamic_extent*/> class span {
   public:
    ...
    template <class Container span_REQUIRES_T((std::is_const<element_type>::value &&
    ...
    span_constexpr const_reverse_iterator crbegin() const span_noexcept { return const_reverse_iterator(cend()); }

    span_constexpr const_reverse_iterator crend() const span_noexcept { return const_reverse_iterator(cbegin()); }
...
inline span_constexpr span<const std17::byte, BytesExtent<T, Extent>::value> as_bytes(span<T, Extent> spn)
    span_noexcept {
#if 0
    return { reinterpret_cast< std17::byte const * >( spn.data() ), spn.size_bytes() };
#else
    return span<const std17::byte, BytesExtent<T, Extent>::value>(reinterpret_cast<std17::byte const*>(spn.data()),
                                                                  spn.size_bytes());  // NOLINT
#endif
}

template <class T, extent_t Extent>
inline span_constexpr span<std17::byte, BytesExtent<T, Extent>::value> as_writable_bytes(span<T, Extent> spn)
    span_noexcept {
#if 0
...
template <class T, std::size_t N>
inline span_constexpr span<const T, static_cast<extent_t>(N)> make_span(std::array<T, N> const& arr) span_noexcept {
...
template <class Container>
inline span_constexpr span<typename Container::value_type> make_span(with_container_t, Container& cont) span_noexcept {
