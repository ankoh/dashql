...
with tpch as property graph (
	vertex tables (
		customers key (c_custkey)
			label Customer properties (c_name as display_name),
		orders key (o_orderkey)
			label PurchaseOrder properties ('Order ' || o_orderkey::text as display_name),
		lineitem key (l_orderkey, l_linenumber)
			label LineItem properties (
				'Line ' || l_orderkey::text || '/' || l_linenumber::text as display_name
			)
	)
	edge tables (
...
from graph_table(
	tpch match
		(customer:Customer where customer.display_name = 'Customer#000000001')
		-[:CommerceStep]->
		(customer_order:PurchaseOrder)
		-[:CommerceStep]->
		(:LineItem)
	one row per step(step_source, step_edge, step_destination)
	columns (
		customer.display_name as path_customer,
		customer_order.display_name as path_order,
		step_source.display_name as source_name,
		step_edge.step_type as step_type,
