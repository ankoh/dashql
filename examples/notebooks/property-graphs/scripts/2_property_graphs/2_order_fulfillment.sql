...
with tpch as property graph (
	...
	edge tables (
		orders as placed_order key (o_orderkey)
			...
			label PlacedOrder no properties,
		lineitem as contains_line key (l_orderkey, l_linenumber)
			...
			label ContainsLine no properties,
		lineitem as supplied_by key (l_orderkey, l_linenumber)
			...
			label SuppliedBy no properties
	)
)
select customer_name, order_key, line_number, supplier_name, net_price
from graph_table(
	tpch match
		(customer:Customer where customer.market_segment = 'BUILDING')
		-[:PlacedOrder]->
		(customer_order:PurchaseOrder where customer_order.order_date < date '1995-03-15')
		-[:ContainsLine]->
		(item:LineItem where item.ship_date > date '1995-03-15')
		-[:SuppliedBy]->
		(vendor:Supplier)
	columns (
		...
		vendor.supplier_name as supplier_name,
