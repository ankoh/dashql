...
EMSCRIPTEN_PREFIX = normalize_path(path_from_root())
...
class Prefixes:
  def __init__(self, args, base_path=None, preserve_deterministic_prefix=True):
    ...
    for p in args:
      if '=' in p:
        ...
        prefixes.append({'prefix': normalize_path(prefix), 'replacement': replacement})
      else:
        prefixes.append({'prefix': normalize_path(p), 'replacement': ''})
    self.base_path = normalize_path(base_path) if base_path is not None else None
  ...
  def resolve(self, name):
    ...
    if not self.preserve_deterministic_prefix and name.startswith(DETERMINISTIC_PREFIX):
      source = EMSCRIPTEN_PREFIX + removeprefix(name, DETERMINISTIC_PREFIX)

    provided = False
    for p in self.prefixes:
      if source.startswith(p['prefix']):
        source = p['replacement'] + removeprefix(source, p['prefix'])
        provided = True
        break
...
def build_sourcemap(entries, code_section_offset, options):
  ...
  for entry in entries:
    ...
    file_name = normalize_path(entry['file'])
