...
mod test {
    ...
    fn ast_dumps() -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        for dump_path in dump_paths {
            ...
            let dump_name_os = dump_path.file_name();
            let dump_name_str = dump_name_os.to_str().unwrap_or_default();

            let xml_suffix = ".xml";
            let tpl_suffix = ".tpl.xml";
            if !dump_name_str.ends_with(xml_suffix) || dump_name_str.ends_with(tpl_suffix) {
                continue;
            }

            let input_file = fs::File::open(&dump_path.path())?;
                    ...
                    Event::Text(t) => {
                        if awaiting_input {
                            if input_text.is_none() {
                                let unescaped =
                                    t.unescape_and_decode(&xml_reader).unwrap_or_default();
