sql_create_as_stmt:
    CREATE_P sql_opt_temp TABLE sql_create_as_target AS sql_select_stmt sql_opt_with_data {
        $$ = ctx.Object(@$, buffers::parser::NodeType::OBJECT_SQL_CREATE_AS, Concat(std::move($4), {
            Attr(Key::SQL_CREATE_AS_TEMP, $2),
            Attr(Key::SQL_CREATE_AS_STATEMENT, ctx.Object(@6, buffers::parser::NodeType::OBJECT_SQL_SELECT, std::move($6))),
  ...
  | CREATE_P sql_opt_temp TABLE IF_P NOT EXISTS sql_create_as_target AS sql_select_stmt sql_opt_with_data {
        $$ = ctx.Object(@$, buffers::parser::NodeType::OBJECT_SQL_CREATE_AS, Concat(std::move($7), {
            Attr(Key::SQL_CREATE_AS_IF_NOT_EXISTS, Bool(Loc({@4, @5, @6}), true)),
            Attr(Key::SQL_CREATE_AS_TEMP, $2),
            Attr(Key::SQL_CREATE_AS_STATEMENT, ctx.Object(@9, buffers::parser::NodeType::OBJECT_SQL_SELECT, std::move($9))),
...
sql_create_stmt:
    CREATE_P sql_opt_temp TABLE sql_qualified_name LRB sql_opt_table_element_list RRB sql_on_commit_option {
        $$ = ctx.Object(@$, buffers::parser::NodeType::OBJECT_SQL_CREATE, {
...
sql_col_constraint:
    CONSTRAINT sql_name sql_col_constraint_elem {
        ...
        $$ = ctx.Object(@$, buffers::parser::NodeType::OBJECT_SQL_COLUMN_CONSTRAINT, std::move($3));
    }
  | sql_col_constraint_elem { $$ = ctx.Object(@$, buffers::parser::NodeType::OBJECT_SQL_COLUMN_CONSTRAINT, std::move($1)); }
  | sql_col_constraint_attr { $$ = std::move($1); }
  | COLLATE sql_any_name    { $$ = ctx.Object(@$, buffers::parser::NodeType::OBJECT_SQL_COLUMN_CONSTRAINT, {
        Attr(Key::SQL_COLUMN_CONSTRAINT_TYPE, Enum(@$, buffers::parser::ColumnConstraint::COLLATE)),
...
sql_col_constraint_attr:
    DEFERRABLE              { $$ = Enum(@$, buffers::parser::ConstraintAttribute::DEFERRABLE); }
  | NOT DEFERRABLE          { $$ = Enum(@$, buffers::parser::ConstraintAttribute::NOT_DEFERRABLE); }
  | INITIALLY DEFERRED      { $$ = Enum(@$, buffers::parser::ConstraintAttribute::INITIALLY_DEFERRED); }
  | INITIALLY IMMEDIATE     { $$ = Enum(@$, buffers::parser::ConstraintAttribute::INITIALLY_IMMEDIATE); }
    ;
...
sql_def_elem:
    sql_col_label EQUALS sql_def_arg {
        $$ = ctx.Object(@$, buffers::parser::NodeType::OBJECT_SQL_DEF_ARG, {
...
sql_numeric_only:
  ...
  | PLUS FCONST          { $$ = Const(@$, buffers::parser::AConstType::FLOAT); }
  | MINUS FCONST          { $$ = Const(@$, buffers::parser::AConstType::FLOAT); }
...
sql_col_constraint_elem:
    NOT NULL_P                { $$ = ctx.List({ Attr(Key::SQL_COLUMN_CONSTRAINT_TYPE, Enum(@$, buffers::parser::ColumnConstraint::NOT_NULL)) }); }
  | NULL_P                    { $$ = ctx.List({ Attr(Key::SQL_COLUMN_CONSTRAINT_TYPE, Enum(@$, buffers::parser::ColumnConstraint::NULL_)) }); }
  | UNIQUE sql_opt_definition { $$ = ctx.List({
        Attr(Key::SQL_COLUMN_CONSTRAINT_TYPE, Enum(@$, buffers::parser::ColumnConstraint::UNIQUE)),
        Attr(Key::SQL_COLUMN_CONSTRAINT_DEFINITION, ctx.Array(@2, std::move($2))),
    });
  }
  | PRIMARY KEY sql_opt_definition { $$ = ctx.List({
        Attr(Key::SQL_COLUMN_CONSTRAINT_TYPE, Enum(@$, buffers::parser::ColumnConstraint::PRIMARY_KEY)),
        Attr(Key::SQL_COLUMN_CONSTRAINT_DEFINITION, ctx.Array(@3, std::move($3))),
    });
  }
  | CHECK_P LRB sql_a_expr RRB sql_opt_no_inherit { $$ = ctx.List({
        Attr(Key::SQL_COLUMN_CONSTRAINT_TYPE, Enum(@$, buffers::parser::ColumnConstraint::CHECK)),
  ...
  | DEFAULT sql_b_expr { $$ = ctx.List({
        Attr(Key::SQL_COLUMN_CONSTRAINT_TYPE, Enum(@$, buffers::parser::ColumnConstraint::DEFAULT)),
...
sql_generic_option_elem:
    sql_col_label SCONST {
        $$ = ctx.Object(@$, buffers::parser::NodeType::OBJECT_SQL_GENERIC_OPTION, {
            ...
            Attr(Key::SQL_GENERIC_OPTION_VALUE, Const(@2, sx::parser::AConstType::STRING)),
        });
    }
    ;
...
sql_opt_temp:
    TEMPORARY           { $$ = Enum(@$, buffers::parser::TempType::LOCAL); }
  | TEMP                { $$ = Enum(@$, buffers::parser::TempType::LOCAL); }
  | LOCAL TEMPORARY     { $$ = Enum(@$, buffers::parser::TempType::LOCAL); }
  | LOCAL TEMP          { $$ = Enum(@$, buffers::parser::TempType::LOCAL); }
  | GLOBAL TEMPORARY    { $$ = Enum(@$, buffers::parser::TempType::GLOBAL); }
  | GLOBAL TEMP         { $$ = Enum(@$, buffers::parser::TempType::GLOBAL); }
  | UNLOGGED            { $$ = Enum(@$, buffers::parser::TempType::UNLOGGED); }
  | %empty              { $$ = Null(); }
    ;

sql_on_commit_option: 
    ON COMMIT DROP              { $$ = Enum(@$, buffers::parser::OnCommitOption::DROP); }
  | ON COMMIT DELETE_P ROWS     { $$ = Enum(@$, buffers::parser::OnCommitOption::DELETE_ROWS); }
  | ON COMMIT PRESERVE ROWS     { $$ = Enum(@$, buffers::parser::OnCommitOption::PRESERVE_ROWS); }
  | %empty                      { $$ = Null(); }
  ;
...
sql_table_constraint:
    CONSTRAINT sql_name sql_table_constraint_elem {
        ...
        $$ = ctx.Object(@$, buffers::parser::NodeType::OBJECT_SQL_TABLE_CONSTRAINT, std::move($3));
    }
  | sql_table_constraint_elem {
        $$ = ctx.Object(@$, buffers::parser::NodeType::OBJECT_SQL_TABLE_CONSTRAINT, std::move($1));
    }
    ;
...
sql_table_constraint_elem:
    CHECK_P LRB sql_a_expr RRB sql_table_constraint_attr_list { $$ = ctx.List({
        Attr(Key::SQL_TABLE_CONSTRAINT_TYPE, Enum(@$, buffers::parser::TableConstraint::CHECK)),
  ...
  | UNIQUE sql_existing_index sql_opt_definition sql_table_constraint_attr_list { $$ = ctx.List({
        Attr(Key::SQL_TABLE_CONSTRAINT_TYPE, Enum(@$, buffers::parser::TableConstraint::UNIQUE)),
  ...
  | UNIQUE sql_opt_column_list sql_opt_definition sql_table_constraint_attr_list { $$ = ctx.List({
        Attr(Key::SQL_TABLE_CONSTRAINT_TYPE, Enum(@1, buffers::parser::TableConstraint::UNIQUE)),
  ...
  | PRIMARY KEY sql_opt_column_list sql_opt_definition sql_table_constraint_attr_list { $$ = ctx.List({
        Attr(Key::SQL_TABLE_CONSTRAINT_TYPE, Enum(@$, buffers::parser::TableConstraint::PRIMARY_KEY)),
  ...
  | FOREIGN KEY sql_opt_column_list REFERENCES sql_qualified_name sql_opt_column_list sql_table_constraint_attr_list sql_key_match sql_key_actions { $$ = ctx.List({
        Attr(Key::SQL_TABLE_CONSTRAINT_TYPE, Enum(Loc({@1, @2}), buffers::parser::TableConstraint::FOREIGN_KEY)),
        Attr(Key::SQL_TABLE_CONSTRAINT_COLUMNS, ctx.Array(@3, std::move($3))),
...
sql_key_match:
    MATCH FULL      { $$ = Enum(@$, buffers::parser::KeyMatch::FULL); }
  | MATCH PARTIAL   { $$ = Enum(@$, buffers::parser::KeyMatch::PARTIAL); }
  | MATCH SIMPLE    { $$ = Enum(@$, buffers::parser::KeyMatch::SIMPLE); }
  | %empty          { $$ = Null(); }
    ;
...
sql_key_update:
    ON UPDATE sql_key_action_command { $$ = ctx.Object(@$, buffers::parser::NodeType::OBJECT_SQL_KEY_ACTION, {
        Attr(Key::SQL_KEY_ACTION_TRIGGER, Enum(Loc({@1, @2}), buffers::parser::KeyActionTrigger::UPDATE)),
        Attr(Key::SQL_KEY_ACTION_COMMAND, $3),
    }); }
    ;

sql_key_delete:
    ON DELETE_P sql_key_action_command { $$ = ctx.Object(@$, buffers::parser::NodeType::OBJECT_SQL_KEY_ACTION, {
        Attr(Key::SQL_KEY_ACTION_TRIGGER, Enum(Loc({@1, @2}), buffers::parser::KeyActionTrigger::DELETE)),
        Attr(Key::SQL_KEY_ACTION_COMMAND, $3),
    }); }
    ;

sql_key_action_command:
    NO ACTION     { $$ = Enum(@$, buffers::parser::KeyActionCommand::NO_ACTION); }
  | RESTRICT      { $$ = Enum(@$, buffers::parser::KeyActionCommand::RESTRICT); }
  | CASCADE       { $$ = Enum(@$, buffers::parser::KeyActionCommand::CASCADE); }
  | SET NULL_P    { $$ = Enum(@$, buffers::parser::KeyActionCommand::SET_NULL); }
  | SET DEFAULT   { $$ = Enum(@$, buffers::parser::KeyActionCommand::SET_DEFAULT); }
    ;
...
sql_table_constraint_attr_elem:
    NOT DEFERRABLE        { $$ = Enum(@$, buffers::parser::ConstraintAttribute::DEFERRABLE); }
  | DEFERRABLE            { $$ = Enum(@$, buffers::parser::ConstraintAttribute::DEFERRABLE); }
  | INITIALLY IMMEDIATE   { $$ = Enum(@$, buffers::parser::ConstraintAttribute::DEFERRABLE); }
  | INITIALLY DEFERRED    { $$ = Enum(@$, buffers::parser::ConstraintAttribute::DEFERRABLE); }
  | NOT VALID             { $$ = Enum(@$, buffers::parser::ConstraintAttribute::DEFERRABLE); }
  | NO INHERIT            { $$ = Enum(@$, buffers::parser::ConstraintAttribute::DEFERRABLE); }
    ;
