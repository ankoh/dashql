// The contents of this file were derived from the arrow IPC reader.
...
impl ArrowStreamReader {
    ...
    pub fn try_new(buffer: &[u8]) -> Result<Self, ArrowError> {
        ...
        let ipc_schema: arrow::ipc::Schema = message
            .header_as_schema()
            .ok_or_else(|| ArrowError::IoError("Unable to read IPC message as schema".to_string()))?;
        ...
        match message.header_type() {
            ...
            arrow::ipc::MessageHeader::RecordBatch => {
                let batch = message
                    ...
                    .ok_or_else(|| ArrowError::IoError("Unable to read IPC message as record batch".to_string()))?;
                let buf = reader.next(message.bodyLength() as usize)?;
                arrow::ipc::reader::read_record_batch(&buf, batch, self.schema(), &self.dictionaries_by_id, None)
                    .map(Some)
            }
            arrow::ipc::MessageHeader::DictionaryBatch => {
                let batch = message
                    .header_as_dictionary_batch()
                    .ok_or_else(|| ArrowError::IoError("Unable to read IPC message as dictionary batch".to_string()))?;
                let buf = reader.next(message.bodyLength() as usize)?;
                arrow::ipc::reader::read_dictionary(&buf, batch, &self.schema, &mut self.dictionaries_by_id)?;
