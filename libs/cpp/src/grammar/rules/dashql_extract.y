...
dashql_csv_attribute:
    ENCODING EQUAL dashql_string_value             { $$ = Attr(@$.encode(), AttrKey::CSV_EXTRACT_ENCODING, $3); }
  | HEADER EQUAL dashql_csv_header_value           { $$ = Attr(@$.encode(), AttrKey::CSV_EXTRACT_HEADER, $3); }
  | DELIMITER EQUAL dashql_string_value            { $$ = Attr(@$.encode(), AttrKey::CSV_EXTRACT_DELIMITER, $3); }
  | QUOTE EQUAL dashql_string_value                { $$ = Attr(@$.encode(), AttrKey::CSV_EXTRACT_QUOTE, $3); }
  | DATE FORMAT EQUAL dashql_string_value          { $$ = Attr(@$.encode(), AttrKey::CSV_EXTRACT_DATE_FORMAT, $4); }
  | TIMESTAMP FORMAT EQUAL dashql_string_value     { $$ = Attr(@$.encode(), AttrKey::CSV_EXTRACT_TIMESTAMP_FORMAT, $4); }
    ;

dashql_csv_header_value:
    dashql_boolean_value               { $$ = $1; }
  | LRB dashql_csv_string_list RRB     { $$ = ctx.AddStringArray(@$.encode(), $2); }
