...
pub fn get_enum_text(target: &proto::Node) -> &'static str {
    ...
    match target.node_type() {
        proto::NodeType::ENUM_DASHQL_VIZ_COMPONENT_TYPE => {
            proto::VizComponentType(v as u8).variant_name().unwrap_or_default()
        }
        proto::NodeType::ENUM_DASHQL_DECLARE_COMPONENT_TYPE => {
            proto::InputComponentType(v as u8).variant_name().unwrap_or_default()
        }
        proto::NodeType::ENUM_DASHQL_IMPORT_METHOD_TYPE => {
            proto::ImportMethodType(v as u8).variant_name().unwrap_or_default()
        }
        proto::NodeType::ENUM_DASHQL_LOAD_METHOD_TYPE => {
            proto::LoadMethodType(v as u8).variant_name().unwrap_or_default()
        }
