...
impl DatabaseInstance {
    pub async fn close(&self) -> Result<(), String> {
        ext_close(self.inner.clone())
            .await
            .map_err(|e| e.as_string().unwrap_or_default())?;
        Ok(())
    }
    pub async fn connect(&self) -> Result<DatabaseConnection, String> {
        let result = ext_create_connection(self.inner.clone())
            .await
            .map_err(|e| e.as_string().unwrap_or_default())?;
...
impl DatabaseConnection {
    pub async fn close(&self) -> Result<(), String> {
        ext_close_connection(self.inner.clone())
            .await
            .map_err(|e| e.as_string().unwrap_or_default())?;
        Ok(())
    }
    pub async fn run_query(&self, text: &str) -> Result<Vec<arrow::record_batch::RecordBatch>, String> {
        let result = ext_run_query(self.inner.clone(), text)
            .await
            .map_err(|e| e.as_string().unwrap_or_default())?;
        let buffer = ext_access_buffer(result.clone()).map_err(|e| e.as_string().unwrap_or_default())?;
        let owned = Uint8Array::from(buffer.clone()).to_vec();
        ext_delete_buffer(result).map_err(|e| e.as_string().unwrap_or_default())?;
