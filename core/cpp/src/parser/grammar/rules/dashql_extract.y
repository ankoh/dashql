dashql_extract_statement:
    EXTRACT dashql_identifier FROM dashql_identifier USING dashql_extract_method {
        $6.push_back(Key::DASHQL_EXTRACT_NAME << String(@2));
        $6.push_back(Key::DASHQL_EXTRACT_DATA << String(@4));
...
dashql_csv_attribute:
    ENCODING '=' SCONST                 { $$ = Key::DASHQL_CSV_EXTRACT_ENCODING << String(@3); }
  | HEADER '=' dashql_csv_header_value  { $$ = Key::DASHQL_CSV_EXTRACT_HEADER << $3; }
  | DELIMITER '=' SCONST                { $$ = Key::DASHQL_CSV_EXTRACT_DELIMITER << String(@3); }
  | QUOTE '=' SCONST                    { $$ = Key::DASHQL_CSV_EXTRACT_QUOTE << String(@3); }
  | DATE FORMAT '=' SCONST              { $$ = Key::DASHQL_CSV_EXTRACT_DATE_FORMAT << String(@4); }
  | TIMESTAMP FORMAT '=' SCONST         { $$ = Key::DASHQL_CSV_EXTRACT_TIMESTAMP_FORMAT << String(@4); }
    ;
...
dashql_csv_string_list:
    dashql_csv_string_list ',' STRING_LITERAL   { $1.push_back(String(@3)); $$ = move($1); }
  | %empty                                      { $$ = {}; }
    ;
