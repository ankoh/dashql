...
namespace {
    ...
    TEST(WebAPITest, ExplainQuery) {
...

        {
            ...
            ASSERT_EQ(opTypes.size(), 2) << std::accumulate(opTypes.begin(), opTypes.end(), string{},
                                                            [](auto& prev, uint8_t type) { return string{prev.empty() ? "" : prev + ","} + proto::engine::LogicalOperatorType_Name(type); });
...

        {
            ...
            auto& opChildOffsets = queryPlan->operator_child_offsets();
            auto& opChildren = queryPlan->operator_children();
            ASSERT_EQ(opTypes.size(), 5) << accumulate(opTypes.begin(), opTypes.end(), string{},
                                                       [](auto& prev, uint8_t type) { return string{prev.empty() ? "" : prev + ","} + proto::engine::LogicalOperatorType_Name(type); });
            ASSERT_EQ(opTypes.Get(0), proto::engine::LogicalOperatorType::OP_PROJECTION);
