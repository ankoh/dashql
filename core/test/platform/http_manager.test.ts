...
describe('HTTPManager', () => {
    ...
    test('fetch', async () => {
        const http = new platform.HTTPManager(2);
        await http.init();

        httpMock
            .onGet("http://localhost/file1").reply(200, encodeTextBody("body1"))
            .onGet("http://localhost/file2").reply(404)
            .onGet("http://localhost/file3").reply(200, encodeTextBody("body3"))
            .onGet("http://localhost/file4").reply(200, encodeTextBody("body4"));
