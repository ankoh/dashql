...
export class TableScanBuffer {
    ...
    /// Constructor
    constructor(db: platform.DatabaseManager, tableName: string, rowCount: number) {
        this._database = db;
    ...
    /// Query a range
    public async queryRange(range: TableScanRange): Promise<[proto.webdb.QueryResult | null, TableScanRange]>  {
    ...
    /// Iterate a range within a table
    public async iterateNumberColumnRange(range: TableScanRange, column: number, fn: (row: number, v: number | null) => void) {
