...
::testing::AssertionResult Matches(c4::yml::ConstNodeRef have, c4::yml::ConstNodeRef expected) {
    std::string have_str = c4::yml::emitrs_yaml<std::string>(have);
    std::string expected_str = c4::yml::emitrs_yaml<std::string>(expected);
...
::testing::AssertionResult MatchesContent(c4::yml::ConstNodeRef have, c4::yml::ConstNodeRef expected_keymap) {
    ...
    std::string expected_full = c4::yml::emitrs_yaml<std::string>(expected_keymap);
...
void EncodeLocation(c4::yml::NodeRef n, buffers::parser::Location loc, std::string_view text, const char* loc_key,
                    const char* text_key) {
    ...
    }
    {
        ...
        text_node << c4::yml::key(text_key) << text_val;
