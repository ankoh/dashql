...
#else
// Fallback for compilers that don't support it
...
template <class Size_T> class SmallVectorBase {
   protected:
    ...
    SmallVectorBase(void* FirstEl, size_t TotalCapacity)
        : begin_x_(FirstEl), capacity_(static_cast<Size_T>(TotalCapacity)) {}
   ...
   protected:
    ...
    void set_allocation_range(void* Begin, size_t N) {
        ...
        begin_x_ = Begin;
        capacity_ = static_cast<Size_T>(N);
    }
};
...
class SmallVectorTemplateCommon : public SmallVectorBase<SmallVectorSizeType<T>> {
   ...
   protected:
    ...
    bool isSmall() const { return this->begin_x_ == getFirstEl(); }

    /// Put this vector in a state of being small.
    void resetToSmall() {
        this->begin_x_ = getFirstEl();
        this->Size = this->capacity_ = 0;  // FIXME: Setting Capacity to 0 is suspect.
    }
    ...
    template <class ItTy> void assertSafeToReferenceAfterClear(ItTy From, ItTy To) {
    ...
    template <class U> static const T* reserveForParamAndGetAddressImpl(U* This, const T& Elt, size_t N) {
   ...
   protected:
    ...
    template <typename It1, typename It2> static void uninitialized_copy(It1 I, It1 E, It2 Dest) {
        if constexpr (std::is_pointer_v<It1> && std::is_pointer_v<It2> &&
            ...
            if (I != E) std::memcpy(reinterpret_cast<void*>(Dest), I, (E - I) * sizeof(T));
        } else {
...
extern template class dashql::SmallVectorBase<uint32_t>;
#if SIZE_MAX > UINT32_MAX
extern template class dashql::SmallVectorBase<uint64_t>;
#endif
...
template <typename T> inline void swap(dashql::SmallVectorImpl<T>& LHS, dashql::SmallVectorImpl<T>& RHS) {
    LHS.swap(RHS);
}

/// Implement std::swap in terms of SmallVector swap.
template <typename T, unsigned N> inline void swap(dashql::SmallVector<T, N>& LHS, dashql::SmallVector<T, N>& RHS) {
    LHS.swap(RHS);
}
