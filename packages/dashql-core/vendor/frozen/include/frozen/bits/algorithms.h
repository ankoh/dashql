...
template <typename Iter> constexpr void iter_swap(Iter a, Iter b) { cswap(*a, *b); }
...
template <class T, class Compare> struct LowerBound {
    ...
    inline constexpr ForwardIt doitfirst(ForwardIt first, std::integral_constant<std::size_t, N>,
        ...
        if (compare_(*it, value_)) {
            ...
            return doitfirst(it + 1, std::integral_constant<std::size_t, next>{}, std::integral_constant < bool,
                             next_highest_power_of_two(next) - 1 == next > {});
        } else
...
constexpr ForwardIt lower_bound(ForwardIt first, const T &value, Compare const &compare) {
    return LowerBound<T, Compare>{value, compare}.doitfirst(first, std::integral_constant<std::size_t, N>{},
                                                            std::integral_constant < bool,
                                                            next_highest_power_of_two(N) - 1 == N > {});
}
