...
struct bucket_size_compare {
    template <typename B> bool constexpr operator()(B const &b0, B const &b1) const { return b0.size() > b1.size(); }
};
...
template <std::size_t M> struct pmh_buckets {
...
template <class T, std::size_t N> constexpr bool all_different_from(cvector<T, N> &data, T &a) {
...
template <std::size_t M, class Hasher> struct pmh_tables : private Hasher {
    std::uint64_t first_seed_;
    carray<seed_or_index, M> first_table_;
    carray<std::size_t, M> second_table_;

    constexpr pmh_tables(std::uint64_t first_seed, carray<seed_or_index, M> first_table,
                         carray<std::size_t, M> second_table, Hasher hash) noexcept
        : Hasher(hash), first_seed_(first_seed), first_table_(first_table), second_table_(second_table) {}

    constexpr Hasher const &hash_function() const noexcept { return static_cast<Hasher const &>(*this); }
    ...
    constexpr std::size_t lookup(const KeyType &key, const HasherType &hasher) const {
        ...
        }  // this is narrowing std::uint64 -> std::size_t but should be fine
        else {
...
pmh_tables<M, Hash> constexpr make_pmh_tables(const carray<Item, N> &items, Hash const &hash, KeyEqual const &equal,
                                              Key const &key, PRG prg) {
