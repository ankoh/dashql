...
struct GetKey {
    template <class KV> constexpr auto const &operator()(KV const &kv) const { return kv.first; }
};
...
class unordered_map : private KeyEqual {
   ...
   public:
    ...
    /* lookup */
    template <class KeyType> constexpr std::size_t count(KeyType const &key) const { return find(key) != end(); }

    template <class KeyType> constexpr Value const &at(KeyType const &key) const { return at_impl(*this, key); }
    template <class KeyType> constexpr Value &at(KeyType const &key) { return at_impl(*this, key); }
    ...
    template <class KeyType> constexpr bool contains(KeyType const &key) const {
