...
arrow::Result<duckdb::LogicalType> mapArrowTypeToDuckDB(const arrow::DataType& type) {
    switch (type.id()) {
        ...
        case arrow::Type::type::DECIMAL32: {
            auto& decimal = *dynamic_cast<const arrow::Decimal32Type*>(&type);
            return duckdb::LogicalType::DECIMAL(decimal.precision(), decimal.scale());
        }
        case arrow::Type::type::DECIMAL64: {
            auto& decimal = *dynamic_cast<const arrow::Decimal64Type*>(&type);
            return duckdb::LogicalType::DECIMAL(decimal.precision(), decimal.scale());
        }
...
// TODO: Requires full DuckDB runtime - implement later when we have DuckDB linked
arrow::Status convertArrowArrayToDuckDBVector(arrow::Array& in, duckdb::Vector& out) {
    return arrow::Status::NotImplemented("convertArrowArrayToDuckDBVector requires full DuckDB runtime");
}
