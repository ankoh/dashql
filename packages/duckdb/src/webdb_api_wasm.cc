...
using ConnectionHdl = DuckDBWebConnectionHdl;
using BufferHdl = DuckDBWebBufferHdl;

/// Clear the response buffer
void duckdb_web_clear_response() { DuckDBWebWasmResult::Get().Clear(); }
...
#define GET_WEBDB(PACKED)                                              \
    ...
    if (!maybe_webdb.ok()) {                                           \
        DuckDBWebWasmResult::Get().Store(PACKED, maybe_webdb.status()); \
        return;                                                        \
    }                                                                  \
...
void duckdb_web_reset(WASMResponse* packed) {
    GET_WEBDB(*packed);
    DuckDBWebWasmResult::Get().Store(*packed, webdb.Reset());
}
...
void duckdb_web_open(WASMResponse* packed, const char* args) {
    GET_WEBDB(*packed);
    DuckDBWebWasmResult::Get().Store(*packed, webdb.Open(args));
}

/// Get the duckdb version
void duckdb_web_get_version(WASMResponse* packed) {
    GET_WEBDB(*packed);
    DuckDBWebWasmResult::Get().Store(*packed, webdb.GetVersion());
}
...
void duckdb_web_prepared_create(WASMResponse* packed, ConnectionHdl connHdl, const char* script) {
    ...
    DuckDBWebWasmResult::Get().Store(*packed, std::move(r));
}
/// Prepare a query statement
...
void duckdb_web_query_fetch_results(WASMResponse* packed, ConnectionHdl connHdl) {
    ...
    DuckDBWebWasmResult::Get().Store(*packed, r);
}
