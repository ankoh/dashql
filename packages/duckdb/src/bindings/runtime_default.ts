...
export const DefaultRuntime: DuckDBRuntime & {
    filesByPath: Map<string, DefaultRuntimeFile>;
    filesByID: Map<number, DefaultRuntimeFile>;
    nextFileID: number;
} = {
    filesByPath: new Map<string, DefaultRuntimeFile>(),
    filesByID: new Map<number, DefaultRuntimeFile>(),
    nextFileID: 0,
    bindings: null,
    ...
    duckdb_web_add_file_buffer(url: string, buffer: Uint8Array): number {
        const file = DefaultRuntime.filesByPath.get(url);
        if (file) return file.fileID;
        const fileID = DefaultRuntime.nextFileID++;
        const newFile: DefaultRuntimeFile = {
            fileID,
            url,
            buffer,
            lastModified: new Date(),
        };
        DefaultRuntime.filesByPath.set(url, newFile);
        DefaultRuntime.filesByID.set(fileID, newFile);
        return fileID;
    },
    ...
    duckdb_web_get_file_buffer(fileId: number): Uint8Array | null {
        const file = DefaultRuntime.filesByID.get(fileId);
        if (!file) return null;
        return file.buffer;
    },
    duckdb_web_fs_read: function (fileId: number, buf: number, bytes: number, location: number) {
        ...
        const instance = DefaultRuntime.bindings!.instance!;
    ...
    duckdb_web_fs_directory_exists: function (pathPtr: number, pathLen: number) {
        // TODO check if theres any DefaultRuntimeFile with prefix
        return false;
    },
    ...
    duckdb_web_fs_glob: function (pathPtr: number, pathLen: number) {
        ...
        for (const url of DefaultRuntime.filesByPath.keys()) {
            if (re.test(url)) {
                const data = encoder.encode(url);
                const ptr = instance.stackAlloc(data.length);
                instance.HEAPU8.set(data, ptr);
                DefaultRuntime.bindings!.instance!.ccall(
    ...
    duckdb_web_fs_file_open: function (pathPtr: number, pathLen: number, flags: number) {
        const instance = DefaultRuntime.bindings!.instance!;
        const path = decoder.decode(instance.HEAPU8.subarray(pathPtr, pathPtr + pathLen));
        const file = DefaultRuntime.filesByPath.get(path);
        if (file) return file.fileID;
    ...
    duckdb_web_fs_file_move: function (fromPtr: number, fromLen: number, toPtr: number, toLen: number) {
        ...
        const file = DefaultRuntime.filesByPath.get(fromPath);
        if (!file) return;

        file.url = toPath;
        DefaultRuntime.filesByPath.delete(fromPath);
        DefaultRuntime.filesByPath.set(toPath, file);
    },
    ...
    duckdb_web_fs_file_remove: function (pathPtr: number, pathLen: number) {
        const instance = DefaultRuntime.bindings!.instance!;
        const path = decoder.decode(instance.HEAPU8.subarray(pathPtr, pathPtr + pathLen));
        DefaultRuntime.filesByPath.delete(path);
    },
};
