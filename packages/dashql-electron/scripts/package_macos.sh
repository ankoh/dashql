...
STAGE="${TMPDIR:-/tmp}/dashql-electron-package-$$"
trap 'chmod -R u+w "${STAGE}" 2>/dev/null || true; rm -rf "${STAGE}"' EXIT

mkdir -p "${STAGE}/dist" "${STAGE}/renderer"
cp -R "${WORKSPACE_ROOT}/packages/dashql-electron/dist/." "${STAGE}/dist/"
cp "${WORKSPACE_ROOT}/packages/dashql-electron/src/preload.cjs" "${STAGE}/preload.cjs"
...
sed -e "s|\"version\": \"[^\"]*\"|\"version\": \"${VERSION}\"|" \
    "${WORKSPACE_ROOT}/packages/dashql-electron/package.json" > "${STAGE}/package.json"
cp "${WORKSPACE_ROOT}/packages/dashql-electron/electron-builder.yml" "${STAGE}/electron-builder.yml"
cp "${WORKSPACE_ROOT}/packages/dashql-electron/Entitlements.plist" "${STAGE}/Entitlements.plist"
