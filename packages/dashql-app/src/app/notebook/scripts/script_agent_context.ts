...
/// (the feed) because the execution result lives in the connection state, not NotebookScripts.
export type OutputColumnResolver = (scriptKey: number) => OutputColumn[] | null;

/// Input handed to every context contributor.
export interface AgentContextInput {
    /// The full NotebookScripts aggregate (catalog, scripts, ...).
...
export const referencedTablesSchemaContributor: AgentContextContributor = (input) => {
    ...
    const schemas = lookupTableSchemas(input.notebookScripts.connectionCatalog, referenced);
