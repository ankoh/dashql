...
import { StoredConnection, StoredConnectionAuth, StoredConnectionCatalog, StoredWorkbook, StoredWorkbookEntry, StoredWorkbookScript } from './storage_model.js';
...
export const DB = (new dexie.Dexie(DATABASE_NAME)) as dexie.Dexie & {
    ...
    connectionAuth: EntityTable<StoredConnectionAuth, "connectionId">,
    /// The connection auth state
    ...
    /// The workbook scripts
    workbookEntries: EntityTable<StoredWorkbookEntry, "id">,
};

DB.version(DATABASE_VERSION_1).stores({
    /// The connection config
    connections: "++id",
    /// The connection auth params
    connectionAuth: "connectionId",
    ...
    workbookEntries: "++id,workbookId,scriptId",
});
