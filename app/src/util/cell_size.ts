import { Alignment, VisibleCellRange } from 'react-virtualized';
...
export class CellSizeAndPositionManager {
    ...
    _cellSizeAndPositionData: Map<number, SizeAndPositionData> = new Map();
    ...
    getSizeAndPositionOfCell(index: number): SizeAndPositionData {
        ...
        if (index > this._lastMeasuredIndex) {
            ...
            for (var i = this._lastMeasuredIndex + 1; i <= index; i++) {
                ...
                if (size === undefined || isNaN(size)) {
                    ...
                    this._cellSizeAndPositionData.set(i, {
                        offset,
                        size: 0,
                    });
        ...
        return this._cellSizeAndPositionData.get(index) || { offset: 0, size: 0 };
    }

    getSizeAndPositionOfLastMeasuredCell(): SizeAndPositionData {
        return this._lastMeasuredIndex >= 0
            ? this._cellSizeAndPositionData.get(this._lastMeasuredIndex) || { offset: 0, size: 0 }
            : {
                  offset: 0,
                  size: 0,
              };
