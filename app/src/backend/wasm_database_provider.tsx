...
import { useWasmDatabaseLogger, useWasmDatabaseBundleResolver } from './wasm_database_platform';
import { Resolvable, Resolver } from '../utils/resolvable';
...
export const useWasmDatabase = (): Resolvable<duckdb.AsyncDuckDB, duckdb.InstantiationProgress> =>
    React.useContext(setupCtx)!;
export const useWasmDatabaseResolver = (): Resolver<duckdb.AsyncDuckDB> => React.useContext(resolverCtx)!;
...
export const WasmDatabaseProvider: React.FC<DatabaseProps> = (props: DatabaseProps) => {
    const logger = useWasmDatabaseLogger();
    const resolveBundle = useWasmDatabaseBundleResolver();
