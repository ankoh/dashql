...
import { WorkflowSession } from '../../backend/workflow_session';
...
export async function collectTableSchema(
    ...
    const describe = await session.runQuery<{ column_name: arrow.Utf8; column_type: arrow.Utf8; null: arrow.Utf8 }>(
        `describe ${info.tableSchema || 'main'}.${info.tableName}`,
    );
    let column = 0;
    for (const row of describe) {
        columnNames.push(row!.column_name!);
        columnNameMapping.set(row!.column_name!, column++);
        ...
        columnTypes.push(mapType(row!.column_type!));
    }
